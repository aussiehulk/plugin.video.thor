...
def router(params):
	...
	if action is None:
		...
		isUpdate = control.homeWindow.getProperty('thor.updated')
		if isUpdate == 'true':
			control.execute('RunPlugin(plugin://plugin.video.thor/?action=tools_cleanSettings)')
			control.homeWindow.clearProperty('thor.updated')
...
#---My Lists
####################################################
	#from resources.lib.indexers import thorlists
	from resources.lib.menus import navigator
	if action == 'traktlist'				: navigator.Navigator().traktlist()
	...
	from resources.lib.menus import thorstreams
	if action == 'lists'					: thorstreams.indexer().root()
	...
	####################################################
		...
		if action == 'tools_ShowNews':
			...
			if params.get('opensettings') == 'true':
				control.openSettings(params.get('query2'), 'plugin.video.thor')
		...
		elif action == 'tools_contextThorSettings':
			control.openSettings('0.0', 'context.thor')
			...
			elif cleared: control.notification(message='Thor Log File Successfully Cleared')
			else: control.notification(message='Error clearing Thor Log File, see kodi.log for more info')
	...
	####################################################
		...
		if action == 'play_Item':
			...
			for i in items:
				...
				url = 'plugin://plugin.video.thor/?action=play_Item&title=%s&year=%s&imdb=%s&tmdb=%s&tvdb=%s&season=%s&episode=%s&tvshowtitle=%s&premiered=%s&meta=%s&select=1' % (
			...
			if rtype == 'movie':
				...
				r = 'plugin://plugin.video.thor/?action=play_Item'
			elif rtype == 'episode':
				from resources.lib.menus import episodes
				meta = params.get('meta')
				...
				r = 'plugin://plugin.video.thor/?action=play_Random&rtype=episode'
			elif rtype == 'show':
				from resources.lib.menus import tvshows
				rlist = tvshows.TVshows().get(url, create_directory=False)
				r = 'plugin://plugin.video.thor/?action=play_Random&rtype=season'
	...
	####################################################
		...
		elif action == 'cache_clearKodiBookmark': # context.thor action call only
