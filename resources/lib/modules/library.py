...
class libmovies:
	...
	def auto_movie_setup(self):
		try:
			...
			# control.add_source('Thor Movies', self.library_folder, source_content, icon)
			library_sources.add_source('Thor Movies', self.library_folder, source_content, 'DefaultMovies.png')
		except:
			log_utils.error()
...
class libtvshows:
	...
	def add(self, tvshowtitle, year, imdb, tmdb, tvdb, range=False):
		try:
			...
			try:
				from resources.lib.indexers import seasons, episodes
...
class libepisodes:
	...
	def update(self):
		...
		try:
			...
			dbcur.connection.commit()
			from resources.lib.indexers import seasons as seasonsX
