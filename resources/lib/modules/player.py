...
##################
						...
						if self.enable_playnext and not self.play_next_triggered:
							if int(control.playlist.size()) > 1:
								if self.preScrape_triggered == False:
									xbmc.executebuiltin('RunPlugin(plugin://plugin.video.thor/?action=play_preScrapeNext)')
								...
								if remaining_time < (self.playnext_time + 1) and remaining_time != 0:
									xbmc.executebuiltin('RunPlugin(plugin://plugin.video.thor/?action=play_nextWindowXML)')
	...
	def onAVStarted(self):
		...
		xbmc.log('[ plugin.video.thor ] onAVStarted callback', LOGINFO)

	def onPlayBackStarted(self):
		control.sleep(5000)
		if self.av_started: return xbmc.log('[ plugin.video.thor ] onPlayBackStarted callback', LOGINFO)
		for i in range(0, 500):
		...
		if control.setting('subtitles') == 'true':
			Subtitles().get(self.name, self.imdb, self.season, self.episode)
		xbmc.log('[ plugin.video.thor ] onPlayBackStarted callback', LOGINFO)

	def onPlayBackStopped(self):
		try:
			control.homeWindow.clearProperty('thor.preResolved_nextUrl')
			if self.media_length == 0: return xbmc.log('[ plugin.video.thor ] onPlayBackStopped callback', LOGINFO)
		...
		xbmc.log('[ plugin.video.thor ] onPlayBackStopped callback', LOGINFO)
	...
	def onPlayBackEnded(self):
		...
		# if control.setting('crefresh') == 'true':
			...
			# control.sleep(500)
		# control.trigger_widget_refresh() # skinshortcuts handles widget refresh
		xbmc.log('[ plugin.video.thor ] onPlayBackEnded callback', LOGINFO)

	def onPlayBackError(self):
		control.homeWindow.clearProperty('thor.preResolved_nextUrl')
		Bookmarks().reset(self.current_time, self.media_length, self.name, self.year)
		log_utils.error()
		xbmc.log('[ plugin.video.thor ] onPlayBackError callback', LOGINFO)
		sysexit(1)
	...
	def prescrapeNext(self):
		try:
			if control.playlist.size() > 0 and control.playlist.getposition() != (control.playlist.size() - 1):
				...
				if not self.isPlayingVideo(): return control.homeWindow.clearProperty('thor.preResolved_nextUrl')
				sources.Sources().preResolve(next_sources, next_meta)
			else: control.homeWindow.clearProperty('thor.preResolved_nextUrl')
		except:
			log_utils.error()
			control.homeWindow.clearProperty('thor.preResolved_nextUrl')
