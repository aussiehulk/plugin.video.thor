...
LOGDEBUG = 0
# ###--from here down methods print when Thor logging set to "Normal".
LOGINFO = 1
LOGWARNING = 2
LOGERROR = 3
LOGFATAL = 4
...
DEBUGPREFIX = '[COLOR red][ Thor: %s ][/COLOR]'
...
def log(msg, caller=None, level=LOGINFO):
	...
	try:
		...
		if debug_location == '1':
			log_file = joinPath(LOGPATH, 'thor.log')
		...
		traceback.print_exc()
		import xbmc
		xbmc.log('[ plugin.video.thor ] log_utils.log() Logging Failure: %s' % (e), LOGERROR)
...
def error(message=None, exception=True):
	try:
		...
		import xbmc
		xbmc.log('[ plugin.video.thor ] log_utils.error() Logging Failure: %s' % (e), LOGERROR)
...
def clear_logFile():
	...
	try:
		...
		import xbmc
		xbmc.log('[ plugin.video.thor ] log_utils.clear_logFile() Failure: %s' % (e), LOGERROR)
		cleared = False
	return cleared
...
def view_LogFile(name):
	try:
		...
		windows = TextViewerXML('textviewer.xml', addonPath('plugin.video.thor'), heading=heading, text=text)
		windows.run()
		del windows
	except:
		error()
...
def upload_LogFile(name):
	...
	try:
		...
		text = f.read()
		f.close()
		UserAgent = 'Thor %s' % addonVersion('plugin.video.thor')
		...
		if 'key' in response.json():
			...
			select = selectDialog([i[0] for i in list], lang(32196) if name.lower() == 'thor' else lang(32199))
