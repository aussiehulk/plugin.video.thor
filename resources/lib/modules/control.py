...
def setting(id, fallback=None):
	try: settings_dict = jsloads(homeWindow.getProperty('thor_settings'))
...
def make_settings_dict(): # service runs upon a setting change
	try:
		...
		homeWindow.setProperty('thor_settings', jsdumps(settings_dict))
		refresh_playAction()
		refresh_libPath()
...
def getThorVersion():
	return xbmcaddon.Addon('plugin.video.thor').getAddonInfo('version')
...
def artPath():
	theme = appearance()
	return joinPath(xbmcaddon.Addon('plugin.video.thor').getAddonInfo('path'), 'resources', 'artwork', theme)

def genreIconPath():
	theme = appearance()
	return joinPath(xbmcaddon.Addon('plugin.video.thor').getAddonInfo('path'), 'resources', 'artwork', 'genre_media', 'icons')

def genrePosterPath():
	theme = appearance()
	return joinPath(xbmcaddon.Addon('plugin.video.thor').getAddonInfo('path'), 'resources', 'artwork', 'genre_media', 'posters')
...
def refresh_playAction(): # for thor global CM play actions
	play_mode = setting('play.mode')
	autoPlay = 'true' if play_mode == '1' else ''
	homeWindow.setProperty('thor.autoPlay.enabled', autoPlay)

def refresh_libPath(): # for thor global CM library actions
	homeWindow.setProperty('thor.movieLib.path', transPath(setting('library.movie')))
	homeWindow.setProperty('thor.tvLib.path', transPath(setting('library.tv')))

def refresh_debugReversed(): # called from service "onSettingsChanged" to clear thor.log if setting to reverse has been changed
	if homeWindow.getProperty('thor.debug.reversed') != setting('debug.reversed'):
		homeWindow.setProperty('thor.debug.reversed', setting('debug.reversed'))
		execute('RunPlugin(plugin://plugin.video.thor/?action=tools_clearLogFile)')
