...
def unHideItems(tvdb_ids):
	...
	try:
		...
		for section in sections:
			...
			control.sleep(1000)
		if success:
			if 'plugin.video.thor' in control.infoLabel('Container.PluginName'): control.refresh()
	...
	try:
		...
		if select >= 0:
			if items[select][1] == 'watch':
				...
				control.execute('RunPlugin(plugin://plugin.video.thor/?action=shows_traktHiddenManager)')
			elif items[select][1] == 'unfinishedEpisodeManager':
				control.execute('RunPlugin(plugin://plugin.video.thor/?action=episodes_traktUnfinishedManager)')
			elif items[select][1] == 'unfinishedMovieManager':
				control.execute('RunPlugin(plugin://plugin.video.thor/?action=movies_traktUnfinishedManager)')
