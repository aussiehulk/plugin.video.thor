...
import base64,hashlib,os,random,re,requests,shutil,string,sys,urllib,json,ssl,zipfile, datetime,xbmc,xbmcgui
...
class indexer:
	...
	def root(self):
		try:
			regex.clear()
			url = 'http://aussiehulk.com/MyAddon/venom/main.xml'
			self.list = self.venom_list(url)
	...
	def root_herplace(self):
		try:
			regex.clear()
			url = 'http://aussiehulk.com/MyAddon/venom/herplace.xml'
			self.list = self.venom_list(url)
			for i in self.list: i.update({'content': 'addons'})
			self.addDirectory(self.list)
			return self.list
	...
	def root_kids(self):
		try:
			regex.clear()
			url = 'http://aussiehulk.com/MyAddon/venom/kids/kids_main.xml'
			self.list = self.venom_list(url)
			for i in self.list: i.update({'content': 'addons'})
			self.addDirectory(self.list)
			return self.list
	...
	def root_food(self):
		try:
			regex.clear()
			url = 'http://aussiehulk.com/MyAddon/venom/food/food_main.xml'
			self.list = self.venom_list(url)
			for i in self.list: i.update({'content': 'addons'})
			self.addDirectory(self.list)
			return self.list
	...
	def root_comedy(self):
		try:
			regex.clear()
			url = 'http://aussiehulk.com/MyAddon/venom/comedy/comedy_main.xml'
			self.list = self.venom_list(url)
			for i in self.list: i.update({'content': 'addons'})
			self.addDirectory(self.list)
			return self.list
	...
	def root_music(self):
		try:
			regex.clear()
			url = 'http://aussiehulk.com/MyAddon/venom/music/music_main.xml'
			self.list = self.venom_list(url)
			for i in self.list: i.update({'content': 'addons'})
			self.addDirectory(self.list)
			return self.list
	...
	def root_1click(self):
		try:
			regex.clear()
			url = 'http://aussiehulk.com/MyAddon/venom/1click/1click_main.xml'
			self.list = self.venom_list(url)
			for i in self.list: i.update({'content': 'addons'})
			self.addDirectory(self.list)
			return self.list
	...
	def root_247(self):
		try:
			regex.clear()
			url = 'http://aussiehulk.com/MyAddon/venom/247/247_main.xml'
			self.list = self.venom_list(url)
			for i in self.list: i.update({'content': 'addons'})
			self.addDirectory(self.list)
			return self.list
	...
	def root_4k(self):
		try:
			regex.clear()
			url = 'http://aussiehulk.com/MyAddon/venom/4k/4k_main.xml'
			self.list = self.venom_list(url)
			for i in self.list: i.update({'content': 'addons'})
			self.addDirectory(self.list)
			return self.list
	...
	def root_3d(self):
		try:
			regex.clear()
			url = 'http://aussiehulk.com/MyAddon/venom/3d/3d_main.xml'
			self.list = self.venom_list(url)
			for i in self.list: i.update({'content': 'addons'})
			self.addDirectory(self.list)
			return self.list
	...
	def root_space(self):
		try:
			regex.clear()
			url = 'http://aussiehulk.com/MyAddon/venom/space/space_main.xml'
			self.list = self.venom_list(url)
			for i in self.list: i.update({'content': 'addons'})
			self.addDirectory(self.list)
			return self.list
	...
	def root_iptv(self):
		try:
			regex.clear()
			url = 'http://aussiehulk.com/MyAddon/venom/iptv/iptv_main.xml'
			self.list = self.venom_list(url)
			for i in self.list: i.update({'content': 'addons'})
			self.addDirectory(self.list)
			return self.list
	...
	def getx(self, url, worker=False):
		try:
			...
			url = regex.resolve(r)
			self.list = self.venom_list('', result=url)
	...
	def developer(self):
		try:
			...
			self.list = self.venom_list('', result=result)
	...
	def addSearch(self, url=None):
		try:
			...
			self.list = [] ; threads = [] 
			for link in links: threads.append(workers.Thread(self.venom_list, link))
			...
			for i in self.list:
				try:
					name = ''
					if not i['vip'] in ['venom TV']: name += '[B]%s[/B] | ' % i['vip'].upper()
					name += i['name']
					i.update({'name' : name})
				except:
					pass
	...
	def venom_list(self, url, result=None, enc=False):
		try:
	...
	def movie_info(self, i):
		try:
			...
			if 'crew' in people and 'writing' in people['crew']:
				writer = ', '.join([writer['person']['name'] for writer in people['crew']['writing'] if writer['job'].lower() in ['writer', 'screenplay', 'auvenom']])
