			...
			if action in self.selection_actions:
				...
				if focus_id == 2060: # listItems
					...
					if chosen_listitem.getProperty('thor.isSelected') == 'true':
						...
						season = chosen_listitem.getProperty('thor.season')
						episode = chosen_listitem.getProperty('thor.episode')
		...
		def builder():
			for count, item in enumerate(self.results, 1):
				try:
					...
					listitem.setProperty('thor.tvshowtitle', tvshowtitle)
					...
					listitem.setProperty('thor.premiered', new_date)
					season = str(item.get('season'))
					listitem.setProperty('thor.season', season)
					episode = str(item.get('episode'))
					listitem.setProperty('thor.episode', episode)
					label = '%s  -  %sx%s' % (tvshowtitle, season, episode.zfill(2))
					listitem.setProperty('thor.label', label)
					# labelProgress = str(round(float(item['progress'] * 100), 1)) + '%'
					labelProgress = str(round(float(item['progress']), 1)) + '%'
					listitem.setProperty('thor.progress', '[' + labelProgress + ']')
					...
					listitem.setProperty('thor.genre', str(item.get('genre', '')))
