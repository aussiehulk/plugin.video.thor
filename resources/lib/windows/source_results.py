...
class SourceResultsXML(BaseDialog):
			...
			if action_id in self.selection_actions:
				...
				if 'UNCACHED' in source:
					...
					self.execute_code('RunPlugin(plugin://plugin.video.thor/?action=cacheTorrent&caller=%s&type=%s&title=%s&items=%s&url=%s&source=%s&meta=%s)' %
											(debrid, link_type, sysname, quote_plus(jsdumps(self.results)), quote_plus(chosen_source.getProperty('thor.url')), quote_plus(source_dict), quote_plus(jsdumps(self.meta))))
					self.selected = (None, '')
				else:
				...
				if cm_action == 'sourceInfo':
					...
					debrid = chosen_source.getProperty('thor.debrid')
					name = chosen_source.getProperty('thor.name')
					hash = chosen_source.getProperty('thor.hash')
					self.execute_code('RunPlugin(plugin://plugin.video.thor/?action=showDebridPack&caller=%s&name=%s&url=%s&source=%s)' %
									(quote_plus(debrid), quote_plus(name), quote_plus(chosen_source.getProperty('thor.url')), quote_plus(hash)))
					self.selected = (None, '')
					...
					self.execute_code('RunPlugin(plugin://plugin.video.thor/?action=download&name=%s&image=%s&source=%s&caller=sources&title=%s)' %
		...
		def builder():
			for count, item in enumerate(self.results, 1):
				try:
					...
					if size_label: listitem.setProperty('thor.size_label', size_label)
					else: listitem.setProperty('thor.size_label', 'NA')
	...
	def set_properties(self):
		try:
			...
			# self.setProperty('thor.title', title)
