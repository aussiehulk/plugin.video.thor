		...
		try:
			if action in self.selection_actions:
				...
				if focus_id == 2040: # listItems
					...
					if chosen_listitem.getProperty('thor.isHidden') == 'true':
						...
						if chosen_listitem.getProperty('thor.isSelected') == '':
							chosen_listitem.setProperty('thor.isSelected', 'true')
		...
		def builder():
			for count, item in enumerate(self.results, 1):
				try:
					...
					listitem.setProperty('thor.tvshowtitle', item.get('tvshowtitle'))
					listitem.setProperty('thor.year', str(item.get('year')))
					listitem.setProperty('thor.isHidden', str(item.get('isHidden')))
					listitem.setProperty('thor.isSelected', str(item.get('isHidden')))
					listitem.setProperty('thor.imdb', item.get('imdb'))
					listitem.setProperty('thor.tmdb', str(item.get('tmdb')))
					listitem.setProperty('thor.tvdb', str(item.get('tvdb')))
					listitem.setProperty('thor.status', item.get('status'))
					listitem.setProperty('thor.watched_count', '(watched ' + str(item.get('watched_count').get('watched')) + ' of ' + str(item.get('watched_count').get('total')) + ')')
					...
					# listitem.setProperty('thor.duration', str(item.get('duration'))) # not used
